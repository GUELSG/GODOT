...
@export var run_speed_damping = 0.75
@export var speed = 150
@export var jump_velocity = -300

@export_group("Stoomping Enemies")
...
func handle_enemy_collision(enemy: Enemy):
	...
	if is_instance_of(enemy, Koopa) and (enemy as Koopa).in_a_shell:
		...
		if angle_of_collision > min_stomp_degree && max_stomp_degree > angle_of_collision:
			enemy.die()
			on_enemy_stoomped()
			spawm_points_label(enemy)
		else:
			die()

func on_enemy_stoomped():
	velocity.y = stomp_y_velocity

func spawm_points_label(enemy):
...
func die():
	if player_mode == PlayerMode.small:
		...
		death_tween.twwwn_callback(func (): get_tree().reload_current_scene())
